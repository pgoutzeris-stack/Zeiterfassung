...
async function quickAddCategory() {
  const name = await rootsPrompt({ title: 'Neue Kategorie', label: 'Name der Kategorie', okLabel: 'Anlegen', icon: 'fa-folder-plus' });
  if (!name) return;
...
async function deleteProject(pid) {
  if (!await rootsConfirm({ title: 'Projekt löschen?', desc: 'Das Projekt und alle Aufgaben & Einträge werden dauerhaft gelöscht.', okLabel: 'Löschen', variant: 'danger', icon: 'fa-trash' })) return;
  const backup = {
...
document.getElementById('resetDataBtn').onclick = async () => {
  if (!await rootsConfirm({ title: 'Alle Daten zurücksetzen?', desc: 'ALLE Daten werden gelöscht und auf den Demo-Stand zurückgesetzt.', okLabel: 'Zurücksetzen', variant: 'danger', icon: 'fa-triangle-exclamation' })) return;
